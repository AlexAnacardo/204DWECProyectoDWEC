document.addEventListener('DOMContentLoaded', () => {
    ...
    const cruzRoja = document.getElementById('cruzRoja');
    
    // Contenedores secundarios donde se puede soltar
    const contenedoresSecundarios = document.querySelectorAll('.contenedorSecundario');
...
function drop(event) {
    ...
    const cruzRoja = document.getElementById(data);
    
    // Verificar que el contenedor es uno secundario
    if (event.target.classList.contains('contenedorSecundario')) {
        // Limpiar el contenido previo en el contenedor secundario
